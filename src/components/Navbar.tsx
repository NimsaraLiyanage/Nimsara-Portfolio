            ...
            <div className="ml-08 flex items-center space-x-8">
              {navItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="text-black dark:text-white hover:text-indigo-600 dark:hover:text-indigo-600 px-2 py-3 rounded-md text-base font-medium transition-colors"
                >
                  {item.label}
                </a>
              ))}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="block text-gray-600 hover:text-indigo-600 px-3 py-2 rounded-md text-base	 transition-colors"
              >
                {item.label}
              </a>
            ))}
